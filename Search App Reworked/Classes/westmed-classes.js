...
class Provider {
  ...
  // Getter for the availability property
  get availability() {
  ...
  // Setter for the availability property
  set availability(availability) {
    console.log("setting availability to " + availability + " for " + this.firstName + " " + this.lastName + "")
    this._availability = availability;
    if (isDataCompatible("availability", availability)) {
      this._availabilityCompatible = true;
    } else {
      this._availabilityCompatible = false;
    }
  }
  ...
  // Define a method parseAvailability to parse the availability string into an array of objects
  parseAvailability(availabilityString) {
    return JSON.parse(availabilityString);
}

  // Define a method createEvents to return an array of events for the provider
  createEvents() {
    console.log(this.availability)
    const events = [];
    const availability = this.parseAvailability(this.availability);
  ...
  providerCard() {
    ...
    providerInfoTab.setAttribute("id", "provider-info-tab");
    ...
    calendarTab.setAttribute("id", "calendar-tab");
    calendarTab.textContent = "Calendar";


    // Create a div for the calednar wrapper
    let availabilityCalendar = calendar(this, new Date().getFullYear(), new Date().getMonth() + 1);
    availabilityCalendar.classList.add("calendar");
    availabilityCalendar.classList.add("inactive");
    ...
    cardBody.classList.add("inactive");
    ...
    // Append the provider info tab to the tabs section.
    tabsSection.appendChild(providerInfoTab);

    // Append the calendar tab to the tabs section.
    tabsSection.appendChild(calendarTab);

    // Append the card header title row to the card header.
    cardHeader.appendChild(cardHeaderTitleRow);

    // Append the gender icon to the card header title row.
    cardHeaderTitleRow.appendChild(this.getGenderIcon());

    // Append the provider title to the card header title row.
    cardHeaderTitleRow.appendChild(cardHeaderProviderTitle);

    // Append the provider first name to the card header title row.
    cardHeaderTitleRow.appendChild(cardHeaderProviderFirstName);

    // Append the provider last name to the card header title row.
    cardHeaderTitleRow.appendChild(cardHeaderProviderLastName);

    // Append the language icon to the provider language element.
    providerLanguage.appendChild(languageIcon.icon.cloneNode(true));

    // Append the provider language element to the card header title row.
    cardHeaderTitleRow.appendChild(providerLanguage);

    // Append the age group to the card header title row.
    cardHeaderTitleRow.appendChild(cardHeaderProviderAgeGroup);

    // Append the "Accepts New Patients" icon to the badge icons section.
    badgeIconsSection.appendChild(this.getAcceptsNewPatientsIcon());

    // Conditionally append the "On Leave" icon to the badge icons section if the provider is on leave.
    this.onLeave ? badgeIconsSection.appendChild(this.getOnLeaveIcon()) : null;

    // Conditionally append the "Departing" icon and text to the badge icons section if the provider is departing.
    ...
    // Conditionally append the "Retired" icon to the badge icons section if the provider is retired.
    this.retired ? badgeIconsSection.appendChild(this.getRetiredIcon()) : null;

    // Conditionally append the "Workers' Compensation" icon to the badge icons section if the provider accepts workers' compensation insurance.
    this.insurances.includes("workerscomp") ? badgeIconsSection.appendChild(workersCompIcon.icon.cloneNode(true)) : null;

    // Append the badge icons section to the card header title row.
    cardHeaderTitleRow.appendChild(badgeIconsSection);

    // Append the specialties row to the card header.
    cardHeader.appendChild(CardHeaderSpecialtiesRow);

    // Append the common visit reasons row to the card header.
    ...
    // Append the availability calendar to the card body.
    providerCard.appendChild(availabilityCalendar);

    // Event listeners for the Provider Card Tabs
    providerInfoTab.addEventListener("click", () => {
      cardBody.classList.toggle("active");
      cardBody.classList.toggle("inactive");
    });

    calendarTab.addEventListener("click", () => {
      availabilityCalendar.classList.toggle("active");
      availabilityCalendar.classList.toggle("inactive");
